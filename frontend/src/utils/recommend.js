const JOINT_ALIASES = {
  knee: ['knee', 'patella', 'tibiofemoral', 'meniscus', 'acl', 'pcl', 'lcl', 'mcl', 'patellar'],
  shoulder: ['shoulder', 'glenohumeral', 'rotator cuff', 'labrum', 'acromion', 'bankart', 'slap', 'acromioclavicular', 'ac joint'],
  hip: ['hip', 'acetabular', 'femoroacetabular', 'fai', 'groin', 'ischial', 'trochanter'],
  wrist: ['wrist', 'carpal', 'distal radius', 'hand', 'scaphoid', 'thumb', 'finger', 'flexor tendon'],
  elbow: ['elbow', 'ulnar collateral', 'ucl', 'epicondyle', 'olecranon', 'biceps distal'],
  ankle: ['ankle', 'talus', 'talar', 'foot drop', 'dorsiflexion', 'achilles', 'peroneal', 'syndesmosis', 'subtalar'],
  spine: ['spine', 'back', 'thoracic', 'lumbar', 'cervical', 'neck', 'c-spine', 'cervical spine', 'disc', 'sciatica', 'radiculopathy'],
...
function containsAny(haystack, needles) {
  // Word-boundary matching to avoid false substring matches (e.g. "neck" hitting "knee")
...
function recommendProcedures(profile, procedures) {
  ...
  const scored = procedures.map((proc) => {
    ...
    // Use word-boundary checks and skip tokens <=2 chars to avoid false matches.
    ...
    allKeywords.forEach((kw) => {
      ...
      const tokens = normalizedKw.split(/\s+/).filter(Boolean);
      for (const token of tokens) {
        if (token.length <= 2 || seenKeywords.has(token)) continue;
